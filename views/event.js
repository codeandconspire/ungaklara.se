	...
	return html`
		<main class="View-main">
			${state.prismic.getByUID('event', state.params.slug, function (err, doc) {
				...
				if (!doc) {
					return html`
						<div class="u-container">${intro.loading({ badge: true, image: true })}</div>
					`;
				}
				...
				if (doc.data.details.length) {
					blocks.push(html`
						<div class="u-md-container u-spaceT7">${factsBox(doc.data.details)}</div>
					`);
				}
				...
				if (collapse) {
								...
								${videos.length > 1
									? html`
											<div class="u-uncontain">
												${grid(
													{ carousel: true },
													videos.map((group) => video(group.video))
												)}
											</div>
									  `
					...
					if (spotify.length) {
						blocks.push(
							spotify.length > 1
								? html`
										<div class="u-uncontain u-spaceV5">${grid({ carousel: true }, spotify)}</div>
								  `
					...
					if (images.length) {
						accordion.push(
							details(
								html`<h2>${text`Images`}</h2>`,
								html`
									<div class="u-uncontain">
										${grid(
											{ carousel: true },
											images.map((image) => grid.cell(image))
										)}
									</div>
								`
							)
					...
					doc.data.team.forEach(function (slice) {
						...
						accordion.push(
							details(html`<h2>${heading}</h2>`, grid(opts, slice.items.map(teamMember)))
						);
					});
					...
					if (accordion.length) {
						...
						blocks.push(html`
							<div class="u-container">
								<div class="Text u-sizeFull u-spaceV5">${accordion}</div>
							</div>
						`);
					}
				} else {
					...
					if (videos.length) {
						...
						if (id) {
							...
							if (hashtag) {
								first = html`
									<div class="u-posRelative">${hashtag} ${video(first, { first: true })}</div>
								`;
							} else {
								first = video(first, { first: true });
							}

							blocks.push(html` <div class="u-narrow">${trailer(bgProps, first, other)}</div> `);
						}
					}
								...
								html`
									<div class="u-bgGrayLight">
										<div class="Text u-paddedBox">
											...
											<strong>
												<a href="${resolve(blurb)}"> ${blurb.data.cta || text`Read more`} </a>
											</strong>
										</div>
									</div>
								`
						...
						blocks.push(html`
							<div class="u-spaceV8">
								<section class="u-narrow u-container" id="${doc.id}-dates">
									<hr class="u-spaceB8" />
									${doc.data.dates_heading.length
										? html`
												<div
													class="Text u-sizeFull u-textCenter ${collapse
														? 'u-spaceB4'
														: 'u-spaceB7'}"
												>
										...
										: null}
									${grid(
										{ slim: true, size: { md: '1of2', xl: '1of3' } },
										dates.slice(0, page * 4).map(function (item, index, list) {
											var prev = (page - 1) * 4;
											...
											return html`<div ${attrs}>
												${ticket(item, () => emit('track:purchase', title))}
											</div>`;
										})
									)}
									${dates.length > page * 4
										? pagination({ href: `${state.href}?page=${page + 1}`, onclick: paginate })
										: null}
								</section>
							</div>
						`);
									...
									? Object.assign(
											{
												...
												sizes: '100vw',
												srcset: srcset(doc.data.image.url, [
													400,
													600,
													800,
													1200,
...
function teamMember(props) {
	...
	return html`
		<article class="Text Text--fat">
			...
			${props.label ? html` <strong class="Text-label"> ${props.label} </strong> ` : null}
			${props.text.length ? html` <div>${asElement(props.text, resolve, serialize)}</div> ` : null}
		</article>
	`;
}
...
function meta(state) {
	return state.prismic.getByUID('event', state.params.slug, (err, doc) => {
		...
		var props = {
			title:
				doc.data.shortname && doc.data.shortname.length
					? asText(doc.data.shortname)
