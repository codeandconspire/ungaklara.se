		...
		<main class="View-main">
			<div class="u-container">
				${state.prismic.getSingle('teachers', function (err, doc) {
					...
					var sections = doc.data.sections
						.reduce(
							function (list, slice) {
					...
					return html`
						<div>
							<header>
								...
								${grid(
									{ size: { md: '1of3' } },
									doc.data.blurbs.map(function (item) {
								...
								html`
									<div ${attrs}>
										<div class="Text u-paddedBox">
											${slice.primary.pointers_heading
												? html`
					...
					return html`
						<div class="u-spaceV6">
							${grid(
								...
								items.map(
									(item) => html`
										<div class="Text">${asElement(item.text, resolve, serialize)}</div>
									`
								)
							)}
						</div>
...
function asCard(doc, color) {
	...
	if (image && image.url) {
		const sources = srcset(image.url, [200, 400, 800, [1600, 'q_70,c_thumb']], {
			transforms: 'c_thumb'
		});
