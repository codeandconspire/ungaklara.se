			...
			${state.prismic.getSingle('events', (err, doc) => {
					...
					if (doc && doc.data.notice.length) {
						notice = html`
							<div class="View-notice">
								<div class="Text">${asElement(doc.data.notice, resolve, serialize)}</div>
							</div>
						`;
					} else if (!doc) {
						notice = html`
				...
				return html`
					<div class="u-container">
						<header>
							${doc
								? intro({ title: asText(doc.data.title), adapt: true })
								: intro.loading({ text: false, adapt: true })}
						</header>
						<nav>
							${tablist({ static: true }, [
								{
						...
						${pages && pages.length === page * PAGE_SIZE
							? pagination({ href: getHrefWithParam('page', page + 1), onclick: onpaginate })
							: null}
					</div>
				`;
			})}
	...
	function list(docs) {
		...
		switch (slug) {
			...
			case 'kalendarium': {
				...
				return calendar(
					docs.reduce(function (dates, doc, index) {
						...
						for (let i = 0, len = doc.data.dates.length; i < len; i++) {
							...
							dates.push(
								Object.assign(
									...
									item,
									{ title, image, date, status, link: resolve(item.link) }
								)
							);
						}
			...
			case 'arkiv': {
				return grid(
					{ ordered: true, size: { xs: '1of2', md: '1of3', lg: '1of4' } },
					docs.map(function (doc, index) {
						...
						return html` <div ${attrs}>${doc ? archived(doc) : card.loading()}</div> `;
					})
				);
			}
			default:
...
function archived(doc, index) {
	...
	return card(props, props.image ? null : html` <div class="u-loading u-aspectPoster"></div> `);
}
