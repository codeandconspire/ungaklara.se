					...
					for (let i = 0, len = doc.data.body.length; i < len; i++) {
						...
						if (el !== blurbs || i === doc.data.body.length - 1) {
							if (blurbs.length) {
								...
								body.push(html` <div class="u-spaceV5">${grid(opts, blurbs)}</div> `);
							}
						...
						badge = html`
							<span>
								<a href="${resolve(doc.data.parent)}">
									${parent.data.shortname && parent.data.shortname.lenght
										? asText(parent.data.shortname)
										: asText(parent.data.title)} </a
								>:
							</span>
						`;
					}
...
function asCard(props) {
	...
	if (props.image && props.image.url) {
		const sources = srcset(props.image.url, [200, 400, 600, 900, [1600, 'q_60,c_thumb']], {
			transforms: 'c_thumb'
		});
