...
app.use(
	get('/media/:type/:transform/:uri(.+)', async function (ctx, type, transform, uri) {
...
app.use(
	get('/api/prismic-preview', async function (ctx) {
		...
		var href = await api.getPreviewResolver(token, documentId).resolve(function (doc) {
			try {
				return resolve(doc);
			} catch (err) {
				return null;
...
app.use(
	get('/scen/arkiv', function (ctx, next) {
