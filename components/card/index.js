...
function card(props = {}, slot) {
	...
	var snippetLength = props.file ? 500 : 170;

	var body = props.body;
	if (body) {
		if (typeof window === 'undefined') {
			if (Array.isArray(body) || body[0] === '<') html`<div class="Card-text Text">${body}</div>`;
			else body = html`<p class="Card-text">${snippet(body, props.truncate || snippetLength)}</p>`;
		} else if (typeof body === 'string') {
			body = html`<p class="Card-text">${snippet(body, props.truncate || snippetLength)}</p>`;
		} else {
	...
	return html`
		<article ${attrs}>
			<div class="Card-everything">
				${cover}
				<div
					class="Card-content ${fill ? 'u-hoverTriggerTarget u-paddedBox' : ''}"
					style="--Card-figure-aspect: 142.8%"
				>
					<div class="Card-body">
						${date && date.text && date.datetime
							? html`
									<time
										class="Card-meta"
										datetime="${JSON.stringify(date.datetime).replace(/"/g, '')}"
									>
										${date.text}
									</time>
							  `
					...
					${props.link ? html` <div class="Card-footer">${link(props.link)}</div> ` : null}
				</div>
			</div>
		</article>
	`;
	...
	return html`
		<article class="Card ${opts.shrink ? 'Card--shrink' : ''} is-loading">
			...
			<div class="Card-content">
				<div class="Card-body">
					${opts.date ? html` <time class="Card-meta">${loader(8)}</time> ` : null}
