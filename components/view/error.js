...
function message(status) {
	switch (status) {
		...
		case 503:
			return html`<p>
				${text`You seem to be offline. Check your network connection and ${html`<a
					href=""
					onclick=${reload}
					>${text`try again`}</a
				>`}.`}
		...
		default:
			return html`<p>
				${text`We apologize, an error has occured on our site. It may be temporary and you could ${html`<a
					href=""
					onclick=${reload}
					>${text`try again`}</a
				>`} or go back to ${html`<a href="/">${text`the homepage`}</a>`}.`}
			</p>`;
	}
