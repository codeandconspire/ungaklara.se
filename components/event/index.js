	...
	return html`
		<div ${attrs}>
			<div class="Event-body">
				${props.label ? html` <span class="u-textLabel"> ${props.label} </span> ` : null}
			...
			<div class="Event-image Event-image--outside">${framed(image)}</div>
		</div>
	`;

	function middleman(type, node, content, children) {
		if (type === Elements.image) {
			return html`<div class="Event-image Event-image--inside">${framed(image)}</div> `;
		}
