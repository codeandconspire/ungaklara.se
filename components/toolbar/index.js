...
module.exports = class Toolbar extends Component {
	...
	createElement(props) {
		...
		return html`
			<nav
				class="Toolbar is-fixed"
				style="--Toolbar-visible: ${this.local.visible};"
				id="${this.local.id}"
			>
				<div class="Toolbar-content u-container">
					${props.heading ? html` <h2 class="Toolbar-heading">${props.heading}</h2> ` : null}
					<ul class="Toolbar-actions">
						${props.actions.map((action) => html` <li class="Toolbar-action">${action}</li> `)}
					</ul>
				</div>
			</nav>
		`;
