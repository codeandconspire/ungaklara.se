			...
			<div class="Ticket-everything u-paddedBox">
				<div class="Ticket-main">
					<div>
						...
						${status
							? html`<span class="Ticket-status Ticket-status--${props.status} u-textLabel"
									>${status}</span
							  >`
							: null}
					</div>
					<time datetime="${JSON.stringify(props.date).replace(/^"|"$/, '')}">
						...
						<br />
						<span class="Ticket-month u-textLabel"
							>${format(props.date, 'MMMM', { locale: sv })}</span
						>
					</time>
				</div>
				<div class="Ticket-details">
					<div>
						<span class="Ticket-detail"
							><span class="Ticket-icon">${symbol.clock()}</span> ${timestamp(props.date)}</span
						>
						<br />
						<span class="Ticket-detail"
							><span class="Ticket-icon">${symbol.location()}</span> ${props.location}</span
						>
						${props.misc
							? html`
									<br />
									<span class="Ticket-detail"
										><span class="Ticket-icon">${symbol.check()}</span> ${props.misc}</span
									>
							  `
							: null}
					</div>
					${props.status !== 3 && props.href
						? html`
								<a
									class="Ticket-link"
									href="${props.href}"
